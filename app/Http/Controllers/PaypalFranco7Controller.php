	...
	public function postPayment(BuyTicketRequest $request)
	{
		...
		if(isset($redirect_url)) {
			...
			try{
				...
				Franco7::whereIn('id', $id)->update(['status' => 1]);
	...
	public function getPaymentStatus(Request $request)
	{
		...
		if(empty($request->input('PayerID')) || empty($request->input('token'))){
			...
			try{
				...
				Franco7::whereIn('id', $asientos_id)->update(['status' => 0]);
				//Se ejecuta la transaccion y se bloquean los boletos
				DB::commit();
		...
		if($transaction_state == 'approved'){
			...
			try{
				...
				DB::rollBack();
				return redirect()->route('eventos.franco-escamilla-morelia-6')->withErrors('A ocurrido un error');
			}

			// Flash::error('Payment Failed');
			return redirect()->route('eventos.franco-escamilla-morelia-6')->withErrors('Pago fallo');
		}
