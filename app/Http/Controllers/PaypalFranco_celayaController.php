...
class PaypalFranco_celayaController extends Controller
{
    ...
    public function postPayment(BuyTicketRequest $request)
    {
        ...
        $item->setName('Franco Escamilla Celaya 23 Junio')  //Nombre del item
        ...
        ->setDescription('Boleto(s) Franco Escamilla Celaya 23 Junio. '.$description);

        $redirect_urls = new RedirectUrls();    //Redirección para ver si se completa la compra
        $redirect_urls->setReturnUrl(route('payment.franco_celaya.status'))
        ->setCancelUrl(route('payment.franco_celaya.status'));
        ...
        if(isset($redirect_url)) {
            ...
            try{
                ...
                $asientos = Franco_celaya::WhereIn('id', $id)->where('status', 1)->get();
                if($asientos->count() > 0){
                    return redirect('eventos/franco-escamilla-celaya')->withErrors('Lo sentimos los boletos ya no estan disponibles');
                }

                //Bloqueamos los boletos
                Franco_celaya::whereIn('id', $id)->update(['status' => 1, 'user' => Auth::user()->id]);
                ...
                DB::rollBack();
                return redirect('eventos/franco-escamilla-celaya')->withErrors('A ocurrido un error db');
            }
        ...
        return redirect('eventos/franco-escamilla-celaya')->withErrors('A ocurrido un error');
    }
    ...
    public function getPaymentStatus(Request $request)
    {
        ...
        if(empty($request->input('PayerID')) || empty($request->input('token'))){
            ...
            try{
                ...
                Franco_celaya::whereIn('id', $asientos_id)->update(['status' => 0, 'user' => NULL]);
            ...
            return redirect('eventos/franco-escamilla-celaya')->withErrors('Proceso de compra cancelado');
        }
        ...
        if($transaction_state == 'approved'){
            ...
            try{
                ...
                Franco_celaya::whereIn('id', $asientos_id)->update(['status' => 2, 'user' => $client_id, 'forma_pago' => 'Paypal', 'transaction_id' => $transaction_id]);
                
                $folio = Franco_celaya::max('folio');
                $i = 1;
                $buydata['folios'] = "";
            ...
            try{
                ...
                Franco_celaya::whereIn('id', $asientos_id)->update(['status' => 1, 'user' => $client_id, 'forma_pago' => 'Paypal', 'transaction_id' => $transaction_state]);
            ...
            // Flash::error('Payment Failed');
            return redirect('eventos/franco-escamilla-celaya')->withErrors('Pago fallo');
        }
