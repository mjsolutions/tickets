...
class PaypalFranco6Controller extends Controller
{
	...
	public function postPayment(BuyTicketRequest $request)
	{
		...
		->setCancelUrl(route('payment.franco6.status'));
		...
		if(isset($redirect_url)) {
			...
			try{
				...
				$asientos = Franco6::WhereIn('id', $id)->where('status', 1)->get();
				if($asientos->count() > 0){
					return redirect('eventos/franco-escamilla-morelia-6')->withErrors('Lo sentimos los boletos ya no estan disponibles');
				}
				...
				DB::rollBack();
				return redirect('eventos/franco-escamilla-morelia-6')->withErrors('A ocurrido un error db');
			}
		...
		return redirect('eventos/franco-escamilla-morelia-6')->withErrors('A ocurrido un error');
	}
	...
	public function getPaymentStatus(Request $request)
	{
		...
		if(empty($request->input('PayerID')) || empty($request->input('token'))){
			...
			return redirect('eventos/franco-escamilla-morelia-6')->withErrors('Proceso de compra cancelado');
		}
		...
		if($transaction_state == 'approved'){
			...
			try{
				...
				Franco6::whereIn('id', $asientos_id)->update(['status' => 2, 'user' => $client_id, 'forma_pago' => 'Paypal', 'transaction_id' => $transaction_id]);
				
				$folio = Franco6::max('folio');
				$i = 1;
				$buydata['folios'] = "";
			...
			try{
				...
				Franco6::whereIn('id', $asientos_id)->update(['status' => 1, 'user' => $client_id, 'forma_pago' => 'Paypal', 'transaction_id' => $transaction_state]);
			...
			// Flash::error('Payment Failed');
			return redirect('eventos/franco-escamilla-morelia-6')->withErrors('Pago fallo');
		}
