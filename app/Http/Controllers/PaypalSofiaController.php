...
class PaypalSofiaController extends Controller
{
	...
	public function postPayment(BuyTicketRequest $request)
	{
		...
		$item->setName('Boleto Sofia Niño de Rivera')	//Nombre del item
		...
		try {		
			...
			if (\config('app.debug')) {
				...
				return redirect()->route('bolematico.sofia')->withErrors('Algo salio mal perdon por el inconveniente');
			}
		}
		...
		if(isset($redirect_url)) {
			...
			try{
				...
				DB::rollBack();
				return redirect()->route('bolematico.sofia')->withErrors('A ocurrido un error db');
			}
		...
		return redirect()->route('bolematico.sofia')->withErrors('A ocurrido un error');
	}
	...
	public function getPaymentStatus(Request $request)
	{
		...
		if(empty($request->input('PayerID')) || empty($request->input('token'))){
			...
			try{
				...
				Sofia::whereIn('id', $asientos_id)->update(['status' => 0]);
				//Se ejecuta la transaccion y se bloquean los boletos
				DB::commit();
			...
			return redirect()->route('bolematico.sofia')->withErrors('Proceso de compra cancelado');
		}
		...
		if($result->getState() == 'approved'){
			...
			try{
				...
				Sofia::whereIn('id', $asientos_id)->update(['confirmacion' => 1, 'user' => $client_id, 'forma_pago' => 'Paypal', 'transaction_id' => $transaction_id]);
				
				$folio = Sofia::max('folio');
				$i = 1;
				
				foreach ($asientos_id as $asiento) {

					Sofia::where('id', $asiento)->update(['folio' => ($folio + $i)]);
					$i++;
				}
